...
model_test = activation_function + "_" + optimizador +"_GAN_LRAnneal"
...
for epoch in range(num_epochs):
    ...
    # Para el learning rate anneal
    lr_now = anneal_lr(lr_now, num_epochs, epoch)
    if optimizador == "SGD": 
        discriminator_optimizer = optim.SGD(discriminator.parameters(), lr=lr_now)
        generator_optimizer = optim.SGD(generator.parameters(), lr=lr_now)
    elif optimizador == "Adam":
        discriminator_optimizer = optim.Adam(discriminator.parameters(), lr=lr_now)
        generator_optimizer = optim.Adam(generator.parameters(), lr=lr_now)
    print("Current learning rate: {:.4f}".format(lr_now))
